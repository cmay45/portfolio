...
export default function Portfolio() {
  ...
  const services = [
    {
      ...
      text: "Clarify business questions, map data sources, identify metric gaps, and define what decisions the data can responsibly support.",
    },
    {
      title: "Analytics Engineering & Platforms",
      text: "Design data marts, pipelines, semantic layers, QA checks, and dashboard-ready infrastructure that reduces manual reporting drag.",
    },
    {
      title: "Measurement & Decision Science",
      text: "Build forecasting, lift analysis, segmentation, scoring, optimization, and causal measurement frameworks for higher-stakes decisions.",
    },
    {
      title: "Production ML & Automation",
      text: "Develop practical ML systems, anomaly detection, propensity models, monitoring workflows, and deployable analytical tools.",
    },
    {
      title: "AI / Data Readiness",
      text: "Assess whether the data, process, and architecture are ready to support AI, automation, Copilot, or predictive modeling initiatives.",
    },
  ];

  const projectGroups = [
    {
      title: "Decision Science & Measurement",
      ids: ["amazon-did", "apparel-brand-leakage", "repurchase-propensity"],
    },
    {
      title: "Forecasting & Optimization",
      ids: ["arima-forecasting", "price-optimization", "call-center"],
    },
    {
      title: "ML Systems & Monitoring",
      ids: ["verizon-anomaly", "anomaly-detection", "construction-cost"],
    },
    {
      title: "Data Platforms & Automation",
      ids: ["paid-media-mart", "lead-routing", "gcp-platform"],
    },
  ];
  ...
  const t = dark ? {
    ...
    navBtnBg: "#26264a", navBtnBorder: "#363660", navBtnText: "#a0a0d0",
    closeBg: "#26264a", closeBorder: "#363660", closeText: "#a0a0d0",
    nameText: "#e8eaf6", subtitleText: "#8890c0", bodyText: "#a4abc8",
    sectionLabel: "#6d78b8", cardTitle: "#dde2f8", cardSubtitle: "#7d8ac6", cardBody: "#b7bfdc",
    modalTagText: "#7080b0", modalTitle: "#e8eaf6", modalBody: "#b0b8d8",
    bulletText: "#b0b8d8", impactText: "#c4cae8", footerText: "#606890",
    ...
    softBg: "#171732", softBorder: "#29294e",
  } : {
    ...
    navBtnBg: "#f2f2f8", navBtnBorder: "#dcdcf0", navBtnText: "#4040a0",
    closeBg: "#f2f2f8", closeBorder: "#dcdcf0", closeText: "#5050a0",
    nameText: "#1a1a2e", subtitleText: "#7070b0", bodyText: "#444468",
    ...
    softBg: "#fbfbff", softBorder: "#e7e7f2",
  };
  ...
  const short = (text, n = 190) => text.length > n ? `${text.slice(0, n).trim()}…` : text;
  ...
  const sectionLabel = (text) => (
    <div style={{ fontSize: 10, letterSpacing: "0.15em", color: t.sectionLabel, textTransform: "uppercase", fontFamily: "'IBM Plex Mono',monospace", marginBottom: 18 }}>
      // {text}
    </div>
  );

  const ProjectCard = ({ p }) => (
    <div className="proj-card" style={{ "--accent": accentText(p.color), background: t.cardBg, border: `1px solid ${t.cardBorder}`, boxShadow: dark ? "none" : "0 1px 4px rgba(0,0,0,0.05)" }} onClick={() => openModal(p.id)}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 12 }}>
        <span style={{ display: "inline-block", padding: "3px 8px", borderRadius: 999, fontSize: 10, fontWeight: 500, letterSpacing: "0.05em", background: t.pillBg, border: `1px solid ${t.pillBorder}`, color: t.pillText, fontFamily: "'IBM Plex Mono',monospace" }}>
          {p.tag.split(" — ")[0].split(" · ")[0]}
        </span>
        <span style={{ width: 9, height: 9, borderRadius: "50%", background: dark ? p.color : accentText(p.color), flexShrink: 0, marginTop: 5, boxShadow: dark ? `0 0 6px ${p.color}` : "none" }} />
      </div>
      <div style={{ fontFamily: "'Fraunces',serif", fontSize: 17, fontWeight: 700, color: t.cardTitle, marginBottom: 7, lineHeight: 1.25 }}>{p.title}</div>
      <div style={{ background: t.impactBg, borderLeft: `3px solid ${accentText(p.color)}`, padding: "9px 12px", borderRadius: "0 4px 4px 0", fontSize: 12, color: t.impactText, lineHeight: 1.55, marginBottom: 12 }}>
        <span style={{ color: accentText(p.color), fontWeight: 700 }}>result: </span>{short(p.impact, 180)}
      </div>
      <div style={{ fontSize: 12.2, color: t.cardBody, lineHeight: 1.65, marginBottom: 12 }}>
        <span style={{ color: t.cardSubtitle }}>problem / solution: </span>{short(p.summary, 150)}
      </div>
      <div>
    ...
    <div style={{ fontFamily: "'IBM Plex Mono','Courier New',monospace", background: t.pageBg, color: t.nameText, minHeight: "100vh", padding: 0, overflowX: "hidden", transition: "background 0.2s, color 0.2s" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .proj-card { border-radius: 8px; padding: 20px; cursor: pointer; transition: all 0.18s ease; position: relative; overflow: hidden; }
        ...
        .jump-link { font-size: 11px; letter-spacing: 0.08em; text-transform: uppercase; opacity: 0.85; }
        .jump-link:hover { opacity: 1; text-decoration: underline; text-underline-offset: 4px; }
        @media (max-width: 720px) {
          .site-header { padding: 24px 24px !important; }
          .site-section { padding-left: 24px !important; padding-right: 24px !important; }
        }
      `}</style>
      ...
      {selected && (
        <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && closeModal()}>
          <div className="modal-box" style={{ background: t.modalBg, border: `1px solid ${accentText(selected.color)}44`, boxShadow: `0 24px 64px rgba(0,0,0,0.22), 0 0 0 1px ${accentText(selected.color)}22` }}>
            ...
            <div style={{ paddingTop: 20 }}>{renderModalImage(selected)}</div>
            <div className="modal-body">
              <div style={{ background: t.impactBg, borderLeft: `3px solid ${accentText(selected.color)}`, padding: "11px 16px", borderRadius: "0 4px 4px 0", fontSize: 12.5, color: t.impactText, marginBottom: 20, fontStyle: "italic" }}>
                <span style={{ color: accentText(selected.color), fontStyle: "normal", fontWeight: 700, marginRight: 6 }}>impact:</span>
                {selected.impact}
              </div>
      ...
      <header id="top" className="site-header" style={{ background: t.headerBg, borderBottom: `1px solid ${t.headerBorder}`, padding: "28px 48px 26px", boxShadow: dark ? "none" : "0 1px 8px rgba(0,0,0,0.04)", transition: "background 0.2s", position: "sticky", top: 0, zIndex: 20 }}>
        <div style={{ maxWidth: 1040, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18, flexWrap: "wrap" }}>
          <a href="#top" style={{ fontFamily: "'Fraunces',serif", fontSize: 23, fontWeight: 700, color: t.nameText, letterSpacing: "-0.01em" }}>Charles May Analytics</a>
          <nav style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center", color: t.subtitleText }}>
            <a className="jump-link" href="#services">Services</a>
            <a className="jump-link" href="#work">Work</a>
            <a className="jump-link" href="#skills">Skills</a>
            <a className="jump-link" href="#about">About</a>
            <a className="jump-link" href="#contact">Contact</a>
      ...
      <main>
        <section className="site-section" style={{ maxWidth: 1040, margin: "0 auto", padding: "58px 48px 36px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.35fr) minmax(260px, 0.65fr)", gap: 28, alignItems: "start" }}>
            <div>
              <div style={{ fontSize: 11, letterSpacing: "0.15em", color: t.sectionLabel, textTransform: "uppercase", marginBottom: 14 }}>Data science · analytics engineering · production ML</div>
              <h1 style={{ fontFamily: "'Fraunces',serif", fontSize: "clamp(36px, 6vw, 64px)", lineHeight: 0.98, letterSpacing: "-0.04em", color: t.nameText, marginBottom: 20 }}>
                Data clarity for leaders who need better decisions, not more dashboards.
              </h1>
              <p style={{ fontSize: 15, color: t.bodyText, lineHeight: 1.85, maxWidth: 720, marginBottom: 22 }}>
                I help organizations clarify complex business problems, build reliable data foundations, and deliver analysis, models, and platforms that support real business decisions.
              </p>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <a href="#work" style={{ padding: "10px 14px", borderRadius: 6, background: dark ? "#e8eaf6" : "#1a1a2e", color: dark ? "#111122" : "#ffffff", fontSize: 12, fontWeight: 700 }}>View selected work</a>
                <a href="#services" style={{ padding: "10px 14px", borderRadius: 6, background: t.softBg, border: `1px solid ${t.softBorder}`, color: t.nameText, fontSize: 12, fontWeight: 700 }}>See services</a>
              </div>
            </div>
            <aside style={{ background: t.softBg, border: `1px solid ${t.softBorder}`, borderRadius: 10, padding: 20 }}>
              <div style={{ fontSize: 10, letterSpacing: "0.15em", color: t.sectionLabel, textTransform: "uppercase", marginBottom: 14 }}>Best fit</div>
              {[
                "Messy reporting environments",
                "Unclear metrics or dashboards nobody trusts",
                "Data platform and data mart design",
                "Forecasting, segmentation, scoring, and lift measurement",
                "AI / ML readiness before buying more tools",
              ].map((item) => (
                <div key={item} style={{ display: "flex", gap: 9, color: t.bodyText, fontSize: 12.5, lineHeight: 1.55, marginBottom: 10 }}>
                  <span style={{ color: dark ? "#64ffda" : "#00695c" }}>→</span>{item}
                </div>
              ))}
            </aside>
          </div>
        ...
        <section className="site-section" style={{ maxWidth: 1040, margin: "0 auto", padding: "18px 48px 34px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))", gap: 12 }}>
            {[
              ["Problem first", "Define the business question before selecting the model, dashboard, or tool."],
              ["Decision-ready", "Produce work that explains assumptions, limitations, and what the business can responsibly do next."],
              ["Built to survive", "Move from notebooks and manual reports toward documented systems, QA, and handoff."],
            ].map(([title, text]) => (
              <div key={title} style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 8, padding: 18 }}>
                <div style={{ fontFamily: "'Fraunces',serif", fontSize: 17, color: t.cardTitle, marginBottom: 8 }}>{title}</div>
                <div style={{ fontSize: 12.5, color: t.cardBody, lineHeight: 1.65 }}>{text}</div>
              </div>
            ))}
          </div>
        </section>

        <section id="services" className="site-section" style={{ maxWidth: 1040, margin: "0 auto", padding: "46px 48px" }}>
          {sectionLabel("services / what i do")}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(245px, 1fr))", gap: 14 }}>
            {services.map((svc) => (
              <div key={svc.title} style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 8, padding: 20 }}>
                <div style={{ fontFamily: "'Fraunces',serif", fontSize: 18, fontWeight: 700, color: t.cardTitle, marginBottom: 10 }}>{svc.title}</div>
                <div style={{ fontSize: 12.5, color: t.cardBody, lineHeight: 1.7 }}>{svc.text}</div>
              </div>
            ))}
          </div>
        </section>

        <section id="work" className="site-section" style={{ maxWidth: 1040, margin: "0 auto", padding: "46px 48px" }}>
          {sectionLabel("selected work — impact first")}
          {projectGroups.map((group) => (
            <div key={group.title} style={{ marginBottom: 36 }}>
              <h2 style={{ fontFamily: "'Fraunces',serif", color: t.nameText, fontSize: 26, marginBottom: 14 }}>{group.title}</h2>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(290px, 1fr))", gap: 14 }}>
                {group.ids.map(id => {
                  const p = projects.find(project => project.id === id);
                  return p ? <ProjectCard key={p.id} p={p} /> : null;
                })}
              </div>
            </div>
          ))}
        ...
        <section id="skills" className="site-section" style={{ maxWidth: 1040, margin: "0 auto", padding: "46px 48px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: 24 }}>
            {Object.entries(skills).map(([group, items]) => (
              <div key={group}>
                ...
                {items.map(item => (
                  <span key={item} style={{ display: "inline-block", padding: "4px 10px", margin: "3px 3px 3px 0", borderRadius: 3, fontSize: 11.5, background: t.skillBg, border: `1px solid ${t.skillBorder}`, color: t.skillText, fontFamily: "'IBM Plex Mono',monospace" }}>{item}</span>
                ))}
              </div>
            ))}
          </div>
        ...
        <section id="about" className="site-section" style={{ maxWidth: 1040, margin: "0 auto", padding: "46px 48px" }}>
          {sectionLabel("about")}
          <div style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 10, padding: 24 }}>
            <p style={{ fontSize: 13.5, color: t.bodyText, lineHeight: 1.85, marginBottom: 14 }}>
              Charlie May is a data scientist, analytics engineer, and data platform builder based in Indianapolis. His work sits at the intersection of business strategy, data engineering, machine learning, and decision science.
            </p>
            <p style={{ fontSize: 13.5, color: t.bodyText, lineHeight: 1.85, marginBottom: 14 }}>
              After 12 years in digital commerce and analytics leadership, Charlie moved deeper into production data science and ML systems while completing an M.S. in Data Analytics from Georgia Tech. His work focuses on problems where backend complexity is often invisible but the business impact is significant: forecasting, anomaly detection, propensity modeling, causal measurement, data marts, automation, and decision infrastructure.
            </p>
            <p style={{ fontSize: 13.5, color: t.bodyText, lineHeight: 1.85 }}>
              Father of five daughters, dog dad to two doodles, and builder of systems that help people make better decisions from messy data.
            </p>
          </div>
        </section>

        <section id="contact" className="site-section" style={{ maxWidth: 1040, margin: "0 auto", padding: "46px 48px 64px" }}>
          <div style={{ background: t.softBg, border: `1px solid ${t.softBorder}`, borderRadius: 12, padding: 28, display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto", gap: 18, alignItems: "center" }}>
            <div>
              <div style={{ fontFamily: "'Fraunces',serif", fontSize: 28, fontWeight: 700, color: t.nameText, marginBottom: 8 }}>Have a messy data problem or unclear business question?</div>
              <div style={{ fontSize: 13, color: t.bodyText, lineHeight: 1.75 }}>Start with the decision you need to make, the systems involved, and what is currently unclear.</div>
            </div>
            <a href="mailto:charliemayandp@gmail.com?subject=Charles%20May%20Analytics%20Inquiry" style={{ padding: "12px 16px", borderRadius: 6, background: dark ? "#e8eaf6" : "#1a1a2e", color: dark ? "#111122" : "#ffffff", fontSize: 12, fontWeight: 700, whiteSpace: "nowrap" }}>Email Charlie</a>
          </div>
          <div style={{ marginTop: 20, fontSize: 11, color: t.footerText, fontFamily: "'IBM Plex Mono',monospace" }}>M.S. Data Analytics — Georgia Tech · Indianapolis, IN</div>
        </section>
      </main>
    </div>
  );
